...
* Two-factor signing (encrypted key + signing token)
* Key revocation list
...
* Signing tokens:    ``nucleus/data/signing_tokens.json``
...
class FileKeyAuthority(KeyAuthority):
    ...
    valid signing token (two-factor).
    """
    ...
    def __init__(
        ...
        signing_tokens_path: str = "nucleus/data/signing_tokens.json",
        ...
        self._signing_tokens_path = Path(signing_tokens_path).resolve()
        self._signing_tokens_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def sign(
        self, agent_id: str, payload: dict, signing_token: str
    ) -> SignedToken:
        """Sign *payload* with *agent_id*'s private key.

        Requires a valid *signing_token* as the second factor.
        """
        # Verify signing token
        if not self._verify_signing_token(agent_id, signing_token):
            raise PermissionError(
                f"Invalid signing token for agent {agent_id!r}"
            )
    ...
    def issue_token(
        ...
        # The issuer signs with their own key — but we need a signing token.
        # For internal issuance, we use a system signing token.
    ...
    def setup_signing_token(self, agent_id: str, token: str) -> None:
        """Register a signing token for *agent_id*.

        The token is stored as a SHA-256 hash; the plaintext is never persisted.
        """
        token_hash = hashlib.sha256(token.encode()).hexdigest()
        tokens = self._load_signing_tokens()
        tokens[agent_id] = token_hash
        self._save_signing_tokens(tokens)

    def _verify_signing_token(self, agent_id: str, token: str) -> bool:
    ...
    def _get_or_create_system_token(self, agent_id: str) -> str:
        """Get or create a system signing token for internal operations."""
        tokens = self._load_signing_tokens()
        ...
        self.setup_signing_token(agent_id, new_token)
        return new_token
    ...
    # ── signing token persistence ──────────────────────────────────────

    def _load_signing_tokens(self) -> dict[str, str]:
        """Load signing token hashes from disk."""
        if not self._signing_tokens_path.exists():
            return {}
        try:
            raw = self._signing_tokens_path.read_text(encoding="utf-8")
            return json.loads(raw)
        except (json.JSONDecodeError, OSError):
            return {}

    def _save_signing_tokens(self, data: dict[str, str]) -> None:
        """Atomically persist signing token hashes."""
        with self._lock:
            payload = json.dumps(data, indent=2)
            tmp = self._signing_tokens_path.with_suffix(
                self._signing_tokens_path.suffix + ".tmp"
            )
            tmp.write_text(payload, encoding="utf-8")
            tmp.rename(self._signing_tokens_path)
