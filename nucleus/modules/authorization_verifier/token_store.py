"""Token store — persistence layer for authorization tokens and revocation list.
...
from .interface import AuthToken
...
class TokenStore:
    """File-backed persistence for authorization tokens and revocation list."""
    ...
    def save_token(self, token: AuthToken) -> None:
        """Persist a authorization token to disk."""
    ...
    def load_token(self, token_id: str) -> AuthToken | None:
        """Load a authorization token from disk, or None if not found."""
    ...
    @staticmethod
    def _token_to_dict(token: AuthToken) -> dict[str, Any]:
        return {
    ...
    @staticmethod
    def _dict_to_token(d: dict[str, Any]) -> AuthToken:
        return AuthToken(
