...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    # Slash commands are explicit user<->bot interactions and always pass the
    # mention gate once they clear the allowed_chats whitelist (not tested here).
    ...
    # entity). We accept it because ``is_command=True`` bypasses mention checks.
    ...
    # Commands dispatched via PTB ``filters.COMMAND`` already target this bot;
    # there is no need to inspect @botname here because PTB filters do that.
    assert adapter._should_process_message(
        _group_message(
            "/status@other_bot",
    ...
    # Commands without bot_command entities still pass when is_command=True.
    assert adapter._should_process_message(
...
def test_slash_command_from_non_whitelisted_chat_is_rejected():
    """A slash command with is_command=True must still be rejected when it arrives
    from a chat not in allowed_chats, even though is_command bypasses the mention gate."""
    adapter = _make_adapter(require_mention=True, allowed_chats=["-200"])

    # chat_id=-201 is NOT in allowed_chats=[-200], so must be rejected
    assert adapter._should_process_message(
        _group_message("/status", chat_id=-201),
        is_command=True,
    ) is False
