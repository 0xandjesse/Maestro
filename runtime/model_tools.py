...
def handle_function_call(
    ...
    try:
        ...
        # ---- Audit log: tool.call (Phase 1, Priority 4) ----
        try:
            import hashlib as _hashlib
            from maestro_transport.audit_log import write as _audit_write, EVENT_TOOL_CALL
            _args_ser = json.dumps(function_args, sort_keys=True, ensure_ascii=False)
            _args_hash = _hashlib.sha256(_args_ser.encode("utf-8")).hexdigest()[:16]
            _audit_write(EVENT_TOOL_CALL, {
                "execution_id": session_id or task_id or None,
        ...
        # ---- Audit log: tool.result (Phase 1, Priority 4) ----
        try:
            from maestro_transport.audit_log import write as _audit_write, EVENT_TOOL_RESULT
            _result_bytes = len(str(result).encode("utf-8")) if result else 0
            _is_success = not (isinstance(result, str) and '"error"' in result)
            _audit_write(EVENT_TOOL_RESULT, {
                "execution_id": session_id or task_id or None,
                "tool_name": function_name,
                "success": _is_success,
                "bytes_out": _result_bytes,
                "duration_ms": duration_ms,
            })
        except Exception:
            pass
        ...
        # ---- Audit log: tool.result (error path) ----
        try:
            from maestro_transport.audit_log import write as _audit_write, EVENT_TOOL_RESULT
            _duration_ms = int((time.monotonic() - _dispatch_start) * 1000) if '_dispatch_start' in locals() else 0
            _audit_write(EVENT_TOOL_RESULT, {
                "execution_id": session_id or task_id or None,
                "tool_name": function_name,
                "success": False,
                "bytes_out": len(error_msg.encode("utf-8")),
