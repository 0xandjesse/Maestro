...
from tools.runtime_registry import runtime_registry  # Phase 1 canonical registry
...
# ---- Execution Caps (hardening Phase 1) ----
# Hard limits applied to every agent turn. Breach = immediate termination.
_DEFAULT_MAX_ITERATIONS = 150
_DEFAULT_MAX_WALL_CLOCK_SECONDS = 1800
_DEFAULT_MAX_TOKEN_BUDGET = 200000
_DEFAULT_MAX_API_COST_CENTS = 500
...
class ExecutionCapsTracker:
    """Tracks and enforces hard execution limits for a conversation loop.
    ...
    def __init__(
        ...
        max_wall_clock_s: int = _DEFAULT_MAX_WALL_CLOCK_SECONDS,
    ...
    def check(self, iteration: int, delta_tokens: int = 0, delta_cost_cents: float = 0.0):
        ...
        if self.max_wall_clock_s:
            elapsed = time.monotonic() - self.wall_clock_start
            if elapsed >= self.max_wall_clock_s:
                raise ExecutionCapBreach("wall_clock_seconds", self.max_wall_clock_s, round(elapsed, 1))
        if self.max_tokens and self.tokens_used >= self.max_tokens:
            raise ExecutionCapBreach("tokens", self.max_tokens, self.tokens_used)
        if self.max_cost_cents and self.cost_cents_used >= self.max_cost_cents:
            raise ExecutionCapBreach("cost_cents", self.max_cost_cents, round(self.cost_cents_used, 2))
        return None


class ExecutionCapBreach(Exception):
    """Raised when an execution invariant is violated."""
    def __init__(self, cap_type: str, limit, actual):
...
class AIAgent:
    ...
    def __init__(
        ...
        # Phase 1 — register in canonical runtime registry
        self._registry_agent_id = None
        try:
            _aid = gateway_session_key or session_id or f"agent-{os.getpid()}-{int(time.time())}"
            runtime_registry.register(
                agent_id=_aid,
                pid=os.getpid(),
                port=0,  # placeholder until gateway runner assigns
                profile=(platform or ""),
                owner=(user_id or ""),
                model=(model or ""),
                status="initializing",
            )
            self._registry_agent_id = _aid
        except Exception:
            pass
    ...
    def close(self) -> None:
        ...
        # Phase 1 — deregister from canonical runtime registry
        if getattr(self, "_registry_agent_id", None):
            try:
                runtime_registry.deregister(self._registry_agent_id)
            except Exception:
                pass
    ...
    def run_conversation(
        ...
        # ---- Audit log: execution.start (Phase 1, Priority 4) ----
        try:
            from maestro_transport.audit_log import write as _audit_write, EVENT_EXECUTION_START
            _audit_write(EVENT_EXECUTION_START, {
                "execution_id": getattr(self, "session_id", "none") or str(uuid.uuid4()),
                "parent_execution_id": getattr(self, "parent_session_id", None),
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # ---- Execution caps hard limits (Phase 1) ----
            if not self._execution_caps_breached and hasattr(self, "execution_caps") and self.execution_caps:
                try:
                    self.execution_caps.check(
                        iteration=api_call_count,
                        delta_tokens=0,
                        delta_cost_cents=0.0,
                    )
                except ExecutionCapBreach as cap_err:
                    self._execution_caps_breached = True
                    logger.warning(
                        "Execution cap breached: %s=%s (actual=%s). Terminating loop.",
                        cap_err.cap_type, cap_err.limit, cap_err.actual,
                    )
                    # Force immediate termination — no graceful summary
                    self.interrupt(f"Execution cap breached: {cap_err.cap_type}")
                    interrupted = True
                    _turn_exit_reason = "cap_breach"
                    # ---- Audit log: cap.breach (Phase 1, Priority 4) ----
                    try:
                        from maestro_transport.audit_log import write as _audit_write, EVENT_CAP_BREACH
                        _audit_write(EVENT_CAP_BREACH, {
                            "execution_id": getattr(self, "session_id", None) or getattr(self, "_session_id", None),
                            "cap_type": cap_err.cap_type,
                            "limit": cap_err.limit,
                            "actual": cap_err.actual,
                        })
                    except Exception:
                        pass
                    # ---- end cap.breach ----
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if hasattr(response, 'usage') and response.usage:
                        ...
                        # Feed usage deltas into execution caps tracker (Phase 1)
                        if not self._execution_caps_breached and hasattr(self, "execution_caps") and self.execution_caps:
                            try:
                                self.execution_caps.check(
                                    iteration=self._api_call_count,
                                    delta_tokens=total_tokens,
                                    delta_cost_cents=(float(cost_result.amount_usd) * 100.0) if cost_result.amount_usd else 0.0,
                                )
                            except ExecutionCapBreach as cap_err:
                                self._execution_caps_breached = True
                                logger.warning(
                                ...
                                try:
                                    ...
                                    pass
                                # ---- end cap.breach ----
                                # The loop will break on next iteration's interrupt check
        ...
        # ---- Audit log: execution.end (Phase 1, Priority 4) ----
        try:
            from maestro_transport.audit_log import write as _audit_write, EVENT_EXECUTION_END
            _wall_clock_s = None
            if hasattr(self, "execution_caps") and self.execution_caps and self.execution_caps.wall_clock_start:
                _wall_clock_s = round(time.monotonic() - self.execution_caps.wall_clock_start, 1)
            _audit_write(EVENT_EXECUTION_END, {
                "execution_id": getattr(self, "session_id", None) or getattr(self, "_session_id", None),
                "iterations": api_call_count,
                "tokens": getattr(self, "session_total_tokens", 0),
                "cost_cents": round(getattr(self, "session_estimated_cost_usd", 0.0) * 100.0, 2),
                "wall_clock_s": _wall_clock_s,
                "outcome": _turn_exit_reason or ("completed" if (completed and not interrupted) else "incomplete"),
            })
        except Exception:
            pass
        # ---- end execution.end ----
