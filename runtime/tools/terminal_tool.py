...
def kill_all_children():
    """Kill all child processes spawned via this tool or process_registry.

    Used by the global kill switch to force-terminate any dangling
    local processes within the 2-second SLA.
    """
    ...
    try:
        ...
        for sess in process_registry.list_sessions():
            try:
                res = process_registry.kill_process(sess.get("id"))
                killed.append({"id": sess.get("id"), "result": res})
            except Exception:
                pass
    except ImportError:
        pass

    # Also send SIGTERM/SIGKILL to our own process group (non-gateway)
    try:
        pgid = os.getpgid(0)
    except (OSError, AttributeError):
        pgid = None
    if pgid is not None and not _is_win:
        try:
            ...
            pass
    return {"killed_sessions": len(killed), "details": killed}
