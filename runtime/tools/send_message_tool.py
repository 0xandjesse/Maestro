...
def _send_via_maestro_bridge(agent_id: str, message: str, *, subject: str = "") -> dict:
    """Send a message to another agent via the Maestro transport network.

    Two-phase delivery:
    1. Look up the target agent in the Maestro registry and POST to their
       transport endpoint. This is the actual agent-to-agent delivery.
    2. Surface a notification to the gateway bridge for human visibility.
    """
    import urllib.request
    if not agent_id:
        return _error("Missing agent_id — use 'maestro:agent_id' format. Example: 'maestro:proteus'")
    if not subject or not subject.strip():
        return _error("Missing subject — Maestro messages require a subject line. Example: subject='Deployment review needed'")

    # --- Maestro Identity Registry Resolver ---
    # 1. Look for identity in the global registry using internal state (if available)
    # Note: In tool-execution context, we derive identity from the environment or profile path.
    # NEVER default to "unknown" — that produces broken Maestro messages with no traceable sender.
    sender_id = None
    
    # Strategy A: HERMES_PROFILE or HERMES_PROFILE_NAME env var (set by gateway)
    ...
    # Strategy B: Derive from HERMES_HOME path (e.g. /home/user/.hermes/profiles/songbird)
    if not sender_id:
    ...
    # Strategy C: Read the registry and match by active gateway PID
    if not sender_id:
        try:
            import subprocess
            my_pid = os.getpid()
            # Walk up process tree to find the gateway parent
            ps_out = subprocess.check_output(["ps", "-o", "pid,ppid,cmd", "-p", str(my_pid)], text=True)
            for line in ps_out.splitlines()[1:]:
                parts = line.split(None, 2)
                if len(parts) >= 3 and "gateway run" in parts[2]:
                    # This is the gateway process - extract profile from cmdline
                    cmd = parts[2]
                    if "--profile" in cmd:
                        profile_part = cmd.split("--profile")[1].strip().split()[0]
                        sender_id = profile_part
                        break
            # If not found, scan all gateway processes and match by parent PID
            if not sender_id:
    ...
    if not sender_id or sender_id == "unknown":
        return _error(
            "Cannot resolve agent identity — sender_id is unresolved. "
            "HERMES_PROFILE/HERMES_PROFILE_NAME not set, HERMES_HOME not set, "
            "and process scan failed. Set HERMES_PROFILE=<agent_name> in the environment."
        ) 

    bridge_url = os.environ.get("MAESTRO_BRIDGE_URL", "http://127.0.0.1:8644/maestro/notify")
    msg_id = f"{sender_id}-{__import__('uuid').uuid4().hex[:8]}"

    # ── Phase 1: Deliver to agent transport ──
    # Ensure we have a clean agent_id (strip 'maestro:' if still present)
    clean_agent_id = agent_id.replace("maestro:", "").strip()
    transport_url = _resolve_agent_transport(clean_agent_id)
    transport_ok = False
    transport_err = None
    if transport_url:
        try:
            delivery_payload = json.dumps({
                ...
                "recipient": clean_agent_id,
                "content": message,
                "subject": subject,
            }).encode()
            req = urllib.request.Request(transport_url, data=delivery_payload,
                                         headers={"Content-Type": "application/json"})
            resp = urllib.request.urlopen(req, timeout=10)
            result = json.loads(resp.read())
            transport_ok = result.get("accepted", False)
        except Exception as e:
            transport_err = str(e)
    else:
        transport_err = f"Agent '{clean_agent_id}' not found in Maestro registry"

    # ── Phase 2: Surface to bridge for human visibility ──
    # Surface to sender's chat (outgoing record)
    bridge_payload_sender = json.dumps({
        "agent_id": sender_id,
        "from": sender_id,
        "to": agent_id,
        "content": message,
    ...
    bridge_ok = False
    try:
        req = urllib.request.Request(bridge_url, data=bridge_payload_sender,
                                     headers={"Content-Type": "application/json"})
        resp = urllib.request.urlopen(req, timeout=10)
        bridge_result = json.loads(resp.read())
        bridge_ok = bridge_result.get("ok", False)
    except Exception:
        pass  # bridge is best-effort; transport delivery is what matters

    # Surface to recipient's chat (incoming notification)
    bridge_payload_recipient = json.dumps({
        "agent_id": agent_id,
        "from": sender_id,
        "to": agent_id,
        "content": message,
    ...
    try:
        req = urllib.request.Request(bridge_url, data=bridge_payload_recipient,
                                     headers={"Content-Type": "application/json"})
        resp = urllib.request.urlopen(req, timeout=10)
        json.loads(resp.read())  # ignore result
    except Exception:
        pass

    if transport_ok:
        return {"ok": True, "message_id": msg_id, "transport": True,
                "bridge": bridge_ok, "delivered_to": agent_id}
    return _error(f"Transport delivery failed for '{agent_id}': {transport_err}")


def _resolve_agent_transport(agent_id: str) -> str | None:
    """Look up an agent's transport endpoint from the Maestro registry."""
    import urllib.request
    registry_path = os.path.expanduser("~/.maestro/registry.json")
    try:
        ...
        pass
    # Fallback: try known port mapping for common agents
    known_ports = {
        "songbird": 3842, "lexicon": 3843, "hermes": 3844,
        "mnemosyne": 3845, "proteus": 3846, "stormtrooper": 3847,
    }
    port = known_ports.get(agent_id)
    if port:
        return f"http://127.0.0.1:{port}/message"
    return None
...
def _handle_send(args):
    ...
    # ── Maestro agent-to-agent messaging ──
    # Route maestro:agent_id through the gateway bridge.
