...
# NOTE: "hermes" is intentionally excluded — it is a valid Swarm Corp officer
# profile name. The CLI binary is called "hermes" but profile loading uses
# HERMES_HOME env var, not --profile, so there is no actual collision.
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (`test`, `tmp`, `root`,
    `sudo`) unless the name is a Swarm Corp officer profile, in which case it
    is permitted. ``default`` is a special pass-through — it's a valid alias
    for the built-in root profile.
    """
    if name == "default":
