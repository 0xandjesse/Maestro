...
def agent_with_key(authority):
    """Set up an agent with a key pair and bearer token.

    Returns (authority, agent_id, keypair, bearer_token).
    """
    agent_id = "test-agent-1"
    keypair = authority.generate_keypair(agent_id)
    bearer = "super-secret-token-12345"
    authority.setup_bearer_token(agent_id, bearer)
...
class TestSignAndVerify:
    ...
    def test_sign_missing_agent(self, authority):
        authority.setup_bearer_token("ghost", "token")
...
class TestKeyRotation:
    ...
    def test_rotate_no_previous_key(self, authority):
        ...
        authority.setup_bearer_token("new-agent", "token")
...
class TestBearerTokens:
    def test_setup_and_verify(self, authority):
        authority.setup_bearer_token("agent-1", "my-secret-token")
        authority.generate_keypair("agent-1")
        signed = authority.sign("agent-1", {"x": 1}, "my-secret-token")
        assert authority.verify(signed).valid

    def test_bearer_token_wrong(self, authority):
        authority.setup_bearer_token("agent-1", "correct-token")
        authority.generate_keypair("agent-1")
        with pytest.raises(PermissionError):
            authority.sign("agent-1", {"x": 1}, "wrong-token")

    def test_bearer_token_persisted(self, authority, tmp_dir):
        authority.setup_bearer_token("agent-1", "my-token")
        authority.generate_keypair("agent-1")
...
class TestMasterKey:
    def test_master_key_persisted(self, tmp_dir):
        ...
        authority1.setup_bearer_token("agent-1", "token")
...
class TestEdgeCases:
    ...
    def test_multiple_agents(self, authority):
        ...
        authority.setup_bearer_token("alice", "alice-token")
        authority.setup_bearer_token("bob", "bob-token")
