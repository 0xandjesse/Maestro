...
class TestDefaultGeneration:
    def test_generate_for_new_agent(self, registry):
        ...
        assert config.display_mode == "long"
...
class TestSetMode:
    def test_set_mode_long_to_short(self, registry):
        registry.generate_for_agent("cee-lo")
        config = registry.set_mode("cee-lo", "short")
        assert config.display_mode == "short"
        # Verify persistence
        visible, mode = registry.check("cee-lo", "inbound")
        assert mode == "short"
    ...
    def test_set_mode_updates_timestamp(self, registry):
        ...
        import time
        time.sleep(0.01)
        c2 = registry.set_mode("cee-lo", "short")
    ...
    def test_set_mode_auto_generates(self, registry):
        ...
        config = registry.set_mode("new-agent", "short")
...
class TestListAll:
    ...
    def test_list_all_reflects_mode_changes(self, registry):
        registry.generate_for_agent("cee-lo")
        registry.set_mode("cee-lo", "short")
        all_configs = registry.list_all()
        cee_lo = next(c for c in all_configs if c.agent_id == "cee-lo")
        assert cee_lo.display_mode == "short"
...
class TestSchemaValidation:
    ...
    def test_missing_agent_id(self):
        errors = validate_config({"display_mode": "long"})
        assert any("agent_id" in e for e in errors)

    def test_empty_agent_id(self):
        errors = validate_config({"agent_id": "", "display_mode": "long"})
        assert any("agent_id" in e for e in errors)
    ...
    def test_duplicate_agent_ids(self):
        errors = validate_registry([
            {"agent_id": "dup", "display_mode": "long"},
            {"agent_id": "dup", "display_mode": "short"},
        ])
...
class TestPersistence:
    ...
    def test_file_is_valid_json(self, tmp_registry_path):
        ...
        for entry in data:
            ...
            assert entry.get("display_mode") in ("long", "short", "off")
