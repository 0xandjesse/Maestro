"""DM Enforcer — file-backed implementation.
...
from key_authority import FileKeyAuthority, SignedToken
...
class FileDMEnforcer(DMEnforcer):
    ...
    def issue_token(
        ...
        # Ensure the issuer has a bearer token for signing
        bearer = self._ensure_bearer_token(issuer_id)
        ...
        # Persist
        self._store.save_token(dm_token)
    ...
    def _ensure_bearer_token(self, agent_id: str) -> str:
        """Ensure *agent_id* has a bearer token for signing operations.

        Returns the plaintext bearer token.
        """
        # Generate a fresh system bearer token and register it
        new_token = secrets.token_hex(32)
        self._ka.setup_bearer_token(agent_id, new_token)
        return new_token
