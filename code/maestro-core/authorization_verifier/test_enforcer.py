"""Tests for the DM Enforcer module.

Covers:
* Policy checks — all DMPolicyResult variants
...
from key_authority import FileKeyAuthority
from registry_manager import FileRegistryManager
...
def key_authority(tmp_dir):
    ...
    return FileKeyAuthority(
        ...
        bearer_tokens_path=os.path.join(tmp_dir, "bearer_tokens.json"),
...
def enforcer(key_authority, registry, tmp_dir):
    """Return a FileDMEnforcer with isolated storage."""
    return FileDMEnforcer(
...
class TestCheckPolicy:
    def test_allows_valid_token(self, enforcer, agents_with_keys, valid_token):
        ...
        result = enforcer.check_policy(bob, alice, valid_token)
        assert result == DMPolicyResult.ALLOWED

    def test_denies_missing_token(self, enforcer, agents_with_keys):
        alice, bob = agents_with_keys
        result = enforcer.check_policy(alice, bob, None)
        assert result == DMPolicyResult.DENIED_NO_TOKEN
    ...
    def test_denies_expired_token(self, enforcer, agents_with_keys):
        ...
        time.sleep(0.01)
        result = enforcer.check_policy(bob, alice, token)
        assert result == DMPolicyResult.DENIED_EXPIRED

    def test_denies_revoked_token(self, enforcer, agents_with_keys, valid_token):
        alice, bob = agents_with_keys
        enforcer.revoke_token(valid_token.token_id)
        result = enforcer.check_policy(bob, alice, valid_token)
        assert result == DMPolicyResult.DENIED_REVOKED
    ...
    def test_denies_missing_send_scope(self, enforcer, agents_with_keys):
        ...
        result = enforcer.check_policy(bob, alice, token)
        assert result == DMPolicyResult.DENIED_SCOPE
    ...
    def test_denies_key_mismatch(self, enforcer, agents_with_keys, valid_token):
        ...
        result = enforcer.check_policy(bob, alice, tampered)
        assert result == DMPolicyResult.DENIED_KEY_MISMATCH
...
class TestIssueToken:
    def test_creates_valid_token(self, enforcer, agents_with_keys):
        ...
        assert isinstance(token, DMToken)
...
class TestRevokeToken:
    ...
    def test_revoked_token_fails_policy(self, enforcer, agents_with_keys, valid_token):
        alice, bob = agents_with_keys
        enforcer.revoke_token(valid_token.token_id)
        result = enforcer.check_policy(alice, bob, valid_token)
        assert result == DMPolicyResult.DENIED_REVOKED
...
class TestTokenLifecycle:
    ...
    def test_multiple_tokens_independent(self, enforcer, agents_with_keys):
        ...
        assert enforcer.check_policy(bob, alice, token1) == DMPolicyResult.DENIED_REVOKED

        # token2 should still be allowed
        assert enforcer.check_policy(bob, alice, token2) == DMPolicyResult.ALLOWED
...
class TestAllPolicyResults:
    """Ensure every DMPolicyResult variant is reachable."""

    def test_allowed(self, enforcer, agents_with_keys, valid_token):
        alice, bob = agents_with_keys
        assert enforcer.check_policy(bob, alice, valid_token) == DMPolicyResult.ALLOWED

    def test_denied_no_token(self, enforcer, agents_with_keys):
        alice, bob = agents_with_keys
        assert enforcer.check_policy(alice, bob, None) == DMPolicyResult.DENIED_NO_TOKEN
    ...
    def test_denied_expired(self, enforcer, agents_with_keys):
        ...
        time.sleep(0.01)
        assert enforcer.check_policy(bob, alice, token) == DMPolicyResult.DENIED_EXPIRED

    def test_denied_revoked(self, enforcer, agents_with_keys, valid_token):
        alice, bob = agents_with_keys
        enforcer.revoke_token(valid_token.token_id)
        assert enforcer.check_policy(bob, alice, valid_token) == DMPolicyResult.DENIED_REVOKED

    def test_denied_scope(self, enforcer, agents_with_keys):
        alice, bob = agents_with_keys
        token = enforcer.issue_token(alice, bob, ["dm:receive"], ttl_hours=24)
        assert enforcer.check_policy(bob, alice, token) == DMPolicyResult.DENIED_SCOPE
    ...
    def test_denied_key_mismatch(self, enforcer, agents_with_keys, valid_token):
        ...
        assert enforcer.check_policy(bob, alice, tampered) == DMPolicyResult.DENIED_KEY_MISMATCH
