    ...
    def register(self, agent_id, webhook_endpoint, capabilities=None, public_key=None):
        with self._lock:
            ...
            try:
                ...
                # Disk always wins over parameter — register() re-reads the file
                # inside fcntl.LOCK_EX, so existing_pk is the authoritative on-disk
                # value. This prevents TOCTOU races when start() passes a stale
                # public_key from an unlocked lookup().
                final_pk = existing_pk or public_key
...
class HermesClient:
    def __init__(self, api_url, api_key, conversation, agent_id=None):
        ...
        # Persistent session ID — officer agents maintain context across messages.
        # The gateway supports X-Hermes-Session-Id for session continuity (see
        # gateway/run.py L15010). Without this, every Maestro message creates a
        # one-shot session where the LLM gets one turn and dies before executing.
        self._session_id = f"maestro-{agent_id or 'agent'}-persistent"
    def _format_prompt(self, message, agent_id=None):
        ...
        *recipient* agent, never as another officer.
        ...
        if isinstance(content, dict):
            content = content.get("text", "") if "text" in content else json.dumps(content, indent=2)
        elif not isinstance(content, str):
            content = str(content)
        # Fallback: if content is empty, try payload.text (standard message format)
        if not content:
            ...
            try:
                ...
                async with ClientSession() as session:
                    async with session.post(
                        ...
                        headers=headers, timeout=ClientTimeout(total=1500)
                    ) as resp:
                        if resp.status != 200:
...
def _sender_str(message, default="unknown"):
    """Safely extract sender string from message, handling both dict and bare-string formats."""
    raw = message.get("sender", "unknown")
    if isinstance(raw, dict):
        return raw.get("agentId", default)
...
class MaestroTransport:
    ...
    def _get_event_bus_url(self) -> str:
        """Get event bus URL from config. ADR-021: transport writes to event bus, not bridge."""
        return self.config.get("eventBusUrl", "http://127.0.0.1:8653/maestro/notify")
    ...
    def __init__(self, config):
        ...
        # Content-based loop breaker: track (sender, content_hash) → deque of timestamps.
        # If the same content appears 3+ times from the same sender in 60s, drop it.
        # This catches ACK loops, echo chains, and any repeating pattern regardless of message type.
        self._content_loop_tracker: dict = defaultdict(deque)  # (sender, content_hash) -> deque of timestamps
        self._LOOP_BREAKER_MAX = 3       # max repeats before drop
        self._LOOP_BREAKER_WINDOW = 60   # seconds
        ...
        # DM permission enforcement — recipient-side capability list (ADR-017)
        # Each agent issues tokens for who may DM them. The transport checks
        # the recipient's issued/ directory — no bearer presentation needed.
        # This is a capability model, not a bearer-token model.
        # Token storage is per-agent: ~/.maestro/dm_tokens/{agent_id}/issued/
        ...
        # Default policy when no tokens have been issued: "closed" (reject all)
        # or "open" (accept all). Per-agent sovereignty — each agent chooses.
        self._dm_default_policy = dm_policy.get("default_policy", "closed")
    ...
    def _check_dm_policy(self, sender_id: str, recipient_id: str) -> Tuple[bool, str]:
        """Check DM permission: has the RECIPIENT issued a capability for the SENDER?

        ADR-017 — recipient-side capability list. The recipient stores tokens
        they've issued in issued/{bearer_id}.json. The sender does not present
        a credential — the transport checks the recipient's own filesystem.
        This is a capability model (like SSH authorized_keys), not a bearer-token model.
    ...
    def _check_dm_policy_legacy(self, sender_id: str, recipient_id: str) -> Tuple[bool, str]:
        """Fallback: check old flat Lexicon-issued tokens during ADR-017 migration.

        The legacy path checks the old flat {sender_id}.json files in the parent
        dm_tokens directory (before per-agent subdirectories were created).
        This is temporary — removed after all agents migrate to per-agent issuance.
        """
        # Legacy tokens live in the parent dm_tokens directory, not per-agent
        legacy_dir = self._dm_token_dir.parent
        token_path = legacy_dir / f"{sender_id}.json"
        if not token_path.exists():
            # Also check received/lexicon.json (migration moved them there)
            token_path = legacy_dir / "received" / "lexicon.json"
            if not token_path.exists():
        ...
        # Legacy: must be Lexicon-issued
        if token.get("issuer_id") != "lexicon":
            return False, "not_lexicon_token"

        # Legacy: check tgt list membership
        payload = token.get("payload", {})
        tgt = payload.get("tgt", [])
    ...
    async def handle_message(self, req):
        ...
        # ── Message-ID dedup (the only content-agnostic dedup) ──
        # Content-based dedup was removed (2026-07-06). It blocked legitimate
        # re-sends — revised ADRs, corrected specs, follow-up directives with
        # similar framing. Message ID is the correct dedup key: same ID = same
        # message. Different ID with similar content = intentional re-send.
        ...
        # ── Content-based loop breaker ──
        # Tracks (sender, content_hash) in a sliding window. If the same content
        # appears 3+ times from the same sender in 60s, drop it. This catches
        # ACK loops, echo chains, and any repeating pattern regardless of msg type.
        # Different from message-ID dedup: different IDs, same content = loop.
        content = message.get("content", "")
        if isinstance(content, str) and content.strip():
            content_hash = hash(content.strip())
            loop_key = (sender, content_hash)
            now_ts = time.time()
            times = self._content_loop_tracker[loop_key]
            # Prune old entries
            while times and (now_ts - times[0]) > self._LOOP_BREAKER_WINDOW:
                times.popleft()
            times.append(now_ts)
            if len(times) >= self._LOOP_BREAKER_MAX:
                log.warning(f"[LOOP_BREAKER] Dropping message from {sender}: same content seen "
                            f"{len(times)} times in {self._LOOP_BREAKER_WINDOW}s "
                            f"(preview: {content.strip()[:80]})")
                asyncio.create_task(self._notify_loop_detected(
                    sender, f"content_loop:{len(times)}x_in_{self._LOOP_BREAKER_WINDOW}s",
                    content.strip()[:200]
                ))
                return web.json_response({"accepted": False, "reason": "loop_breaker:content_repeat",
        ...
        # ── Self-message guard (MUST be before _surface_to_gateway) ──
        # When the agent's own gateway generates a send_message, it arrives
        # back at the transport. The agent already processed it to generate
        # it — re-processing creates a loop: transport → gateway → send_message
        # → transport → gateway → send_message → ...
        # Self-messages should only be surfaced and routed, never re-processed.
        # (Content dedup is handled by the sync dedup above — no need to duplicate.)
        if sender == self.agent_id:
            log.debug(f"Self-message from {sender} — routing, skipping LLM re-processing")
            # Write to work log for audit trail
            self._write_work_log(message)
            # Route to recipient if specified
            recipient = message.get("recipient")
            if isinstance(recipient, dict):
                recipient = recipient.get("agentId", "unknown")
            if recipient and recipient != self.agent_id:
                asyncio.create_task(self._route_to_recipient(message, recipient))
            return web.json_response({"accepted": True, "self_message": True, "agentId": self.agent_id})
        ...
        recipient = message.get("recipient")
        # Normalize recipient to string (same as sender normalization at L938-942)
        if isinstance(recipient, dict):
            recipient = recipient.get("agentId", "unknown")
        elif not recipient:
            recipient = "unknown"

        # -- DM permission gating (ADR-017 — recipient-side capability list) --
        # Always-on when the agent has issued tokens. No boolean toggle.
        # Checks issued/{sender_id}.json first, falls back to legacy flat tokens.
        if msg_type in ("direct", "directive"):
            if sender != "system":
                issued_dir = self._dm_token_dir / "issued"
                has_issued_tokens = issued_dir.exists() and any(issued_dir.iterdir())
                if has_issued_tokens:
                    ok, reason = self._check_dm_policy(sender, recipient)
                    if not ok:
                        # Migration grace period: also check old flat Lexicon tokens
                        ok_legacy, _ = self._check_dm_policy_legacy(sender, recipient)
                        if not ok_legacy:
                ...
                elif self._dm_default_policy == "closed":
                    # No issued tokens + default closed → reject all
                    log.warning(f"DM denied: {sender} → {recipient} (default_closed)")
                    return web.json_response(
                        {"accepted": False, "reason": "dm_denied:default_closed"},
                        status=403,
                    )
                # else: default open — no issued tokens, accept everyone

        # -- Structural message types: handled without LLM --
        # directive: fire-and-forget into the LLM loop. No structural ACK —
        # the agent's completion report IS the acknowledgment. ACKs were
        # removed 2026-07-07 after they caused multi-agent reply loops.
        if msg_type == "directive":
    ...
    async def _route_to_recipient(self, message: dict, recipient: str):
        """Route a self-generated message to its recipient without LLM re-processing."""
        try:
            recipient_reg = self.registry.lookup(recipient)
            if not recipient_reg:
                log.warning(f"Cannot route self-message: recipient {recipient} not in registry")
                return
            endpoint = recipient_reg["webhookEndpoint"]
            async with ClientSession() as s:
                async with s.post(endpoint, json=message, timeout=ClientTimeout(total=10)) as resp:
                    log.info(f"Self-message routed to {recipient}: {resp.status}")
        except Exception as e:
            log.error(f"Self-message routing failed to {recipient}: {type(e).__name__}: {e}")
    ...
    async def _process_bb_search(self, message):
        sender_id = _sender_str(message, "unknown")
        board_id = message.get("boardId", "default")
    ...
    async def _notify_loop_detected(self, sender_id: str, reason: str, content_preview: str):
        ...
        try:
            bridge_url = self._get_event_bus_url()
            payload = {
                "agent_id": self.agent_id,
                "from": self.agent_id,
                "to": "jesse",
    ...
    async def _process_message(self, message):
        try:
            ...
            # --- ADR-017: Policy belongs to actors, not the transport ---
            # Rate limiter, reply-chain suppression, and subject echo suppression
            # were removed (2026-07-06). These were policy decisions — the transport
            # deciding "this looks like a loop, kill it." Agents decide what to
            # respond to. The transport delivers everything.
            sender_id = _sender_str(message, "unknown")
            ...
            # Subject enforcement removed (2026-07-08). With only "long" and "off"
            # visibility modes, the Subject line is metadata, not a delivery requirement.
            # Every message is self-contained — no preview extraction needed.
            ...
            # Outbound mirror removed (2026-07-07). The recipient's transport
            # already surfaces the message when it arrives. Mirroring here
            # creates duplicate events — one from sender, one from recipient.
            # The renderer delivers both = double messages in Telegram.
            sender_id = _sender_str(message)
            sender_reg = self.registry.lookup(sender_id)
            if not sender_reg:
            ...
            # Mark the original message as answered — prevents re-answering
            # the same message and breaks reply chains at depth 2+.
            original_msg_id = message.get("id")
            if original_msg_id:
                self._answered_ids.add(original_msg_id)
                # Also mark the thread root to prevent depth-3+ chains
                thread_root = message.get("inReplyTo")
                if thread_root:
                    self._answered_ids.add(thread_root)
            endpoint = sender_reg["webhookEndpoint"]
    ...
    def _write_work_log(self, message: dict):
        ...
        raw_sender = message.get("sender", "unknown")
        if isinstance(raw_sender, dict):
            sender_str = raw_sender.get("agentId", "unknown")
        else:
            sender_str = str(raw_sender) if raw_sender else "unknown"
        entry = {
            "id": message.get("id"),
            "sender": sender_str,
    ...
    async def _process_memory_write(self, message):
        ...
        agent_id = content.get("agent_id", _sender_str(message, "unknown"))
        requester = _sender_str(message, "")
        if not agent_id:
    ...
    async def _process_memory_read(self, message):
        ...
        requester = content.get("requester", _sender_str(message, ""))
    ...
    async def start(self):
        ...
        for peer_id, peer_url in self.config.get("knownPeers", {}).items():
            # Do NOT pass public_key for peers — register() re-reads inside
            # fcntl.LOCK_EX and preserves whatever is on disk. Passing a stale
            # value from the unlocked lookup() above causes TOCTOU races that
            # wipe publicKeys when multiple transports restart simultaneously.
