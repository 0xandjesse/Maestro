...
if python3 -c "import json; open('$VIS_FILE','w').write(json.dumps({'$PROFILE':{'messages':True,'display_mode':'long','in':True,'out':True}},indent=2))" 2>/dev/null; then
    echo "Maestro messages: $MODE (profile: $PROFILE)"
else
